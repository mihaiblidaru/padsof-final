...
public abstract class Cliente extends Usuario {
	 ...
	 * @param ccard tarjeta de credito del cliente
	 * @param nombre nombre del cliente
	 * @param problemaPagos los problemas de pagos del cliente
	 */
	public Cliente(int id, String identificador, String contrasenya, String ccard, String nombre, boolean problemaPagos) {
	 ...
	 * @param ccard nueva tarjeta de credito del cliente
	 * @return true si se cambia correctamente, si no false
	 * @throws UsuarioNoPermisoException si el usuario no es el admin
	 ...
	 */
	public boolean setCcard(String ccard) throws UsuarioNoPermisoException, SQLException, TablaNoTieneColumnaException, TiposNoCoincidenException {
		...
		try {
			DBManager.updateField(Tabla.CLIENTE, Columna.CLIENTE_CCARD, this.getId(), ccard);/**Actualizamos la base de datos */
			this.ccard = ccard;
	 ...
	 * @param problemaPagos nuevos problemas de pagos del cliente
	 * @return true si se ha cambiado el atributo o false en caso contrario
	 */
	public boolean setProblemaPagos(boolean problemaPagos) {
		try {
			DBManager.updateField(Tabla.CLIENTE, Columna.CLIENTE_PROBLEMA_PAGOS, this.getId(), problemaPagos); /**Actualizamos la base de datos */
			this.problemaPagos = problemaPagos;
			return true;
		}catch(SQLException e) {
	 ...
	 */
	public double getImportePendiente() {return 0;};
	...
	public static Cliente cargarCliente(int id) throws SQLException {
		...
		if(rs.next()) {
			String rol = rs.getString(Columna.CLIENTE_ROL.toString()); /**Conseguimos el rol, dependiendo de e utilizaremos una funcion de carga u otra */ 
	...
	public String toString() {
		return String.format("Cliente [Id=%s, Identificador=%s, ccard=%s, nombre=%s, problemaPagos=%s]",
				getId(), getIdentificador(), ccard, nombre, problemaPagos);
	}
