...
public abstract class Oferta {
	 ...
	 * @param idReserva id de la reserva que se quiere añadir a la lista de reservas
	 ...
	 * @param idReserva el id de la nueva reserva
	 ...
	 * @param inmueble inmueble de la oferta
	 * @param estado el estado de la oferta
	 */
	public Oferta(int id, LocalDate fechaInicio, float precio, float fianza, String descripcion, int inmueble, Estado estado) {
	...
	/**
	 * Devuelve la comision que sera del 2% si es vacacional o del 0,1% si es de vivienda, esta oferta se sobreescribira
	 ...
	 * @param fechaInicio la nueva fecha de inicio de la oferta
	 * @throws TablaNoTieneColumnaException si la tabla no tiene la columna que queremos
	 * @throws TiposNoCoincidenException si los tipos no coinciden
	 * @throws OfertaNoModificableException si la oferta no es modificable
	 * @return true si se cambia correctamente si no false
	 */
	public boolean setFechaInicio(LocalDate fechaInicio) throws OfertaNoModificableException, TablaNoTieneColumnaException, TiposNoCoincidenException {
		if(!this.estado.editable() ) { /**Con esto comprobamos si podemos editar la oferta */ 
			throw new OfertaNoModificableException(this); 
		}
		
		try {
			DBManager.updateField(Tabla.OFERTA, Columna.OFERTA_FECHAINICIO, this.id, fechaInicio);/**Cambiamos la fecha en la base de datos */
			this.fechaInicio = fechaInicio;
			return true;
		}catch(SQLException e) {
	 ...
	 * @return true si se cambia correctamente si no false
	 */
	public boolean setPrecio(float precio) throws OfertaNoModificableException, TablaNoTieneColumnaException, TiposNoCoincidenException {
		if(!this.estado.editable() ) { /**Comprobamos si es editable */
			throw new OfertaNoModificableException(this); 
		}
		
		try {
			DBManager.updateField(Tabla.OFERTA, Columna.OFERTA_PRECIO, this.id, precio); /** Cambiamos el precio en la base de datos */
			this.precio = precio;
			return true;
		}catch(SQLException e) {
	 ...
	 * @param fianza la nueva fianza de la oferta
	 * @throws TablaNoTieneColumnaException si la tabla no tiene la columna que queremos
	 * @throws TiposNoCoincidenException si los tipos no coinciden
	 * @throws OfertaNoModificableException si la oferta no es modificable
	 * @return true si se cambia correctamente si no false
	 */
	public boolean setFianza(float fianza) throws OfertaNoModificableException, TablaNoTieneColumnaException, TiposNoCoincidenException {
		if(!this.estado.editable() ) { /**Comprobamos si es editable */
			throw new OfertaNoModificableException(this); 
		}
		try {
			DBManager.updateField(Tabla.OFERTA, Columna.OFERTA_FIANZA, this.id, fianza); /** Actualizamos la fianza en la base de datos*/
			this.fianza = fianza;
			return true;
		}catch(SQLException e) {
	 ...
	 * @param descripcion la nueva descripcion de la oferta
	 * @throws TablaNoTieneColumnaException si la tabla no tiene la columna que queremos
	 * @throws TiposNoCoincidenException si los tipos no coinciden
	 * @throws OfertaNoModificableException si la oferta no es modificable
	 * @return true si se cambia correctamente si no false
	 */
	public boolean setDescripcion(String descripcion) throws OfertaNoModificableException, TablaNoTieneColumnaException, TiposNoCoincidenException {
		if(!this.estado.editable() ) { /** Comprobamos si es editable*/
			throw new OfertaNoModificableException(this); 
		} 
		try {
			DBManager.updateField(Tabla.OFERTA, Columna.OFERTA_DESCRIPCION, this.id, descripcion); /**Actualizamos en la base de datos */
			this.descripcion = descripcion;
			return true;
		}catch(SQLException e) {
	...
	protected void cargarOpiniones() throws SQLException {
		...
		ResultSet rs = stmt.executeQuery("SELECT * FROM " + Tabla.OPINION + " where " + Columna.OPINION_OFERTA + " = " + this.id);
	 ...
	 * @param estado el nuevo estado de la oferta
	 * @return true si se cambia correctamente si no false
	 * @throws TablaNoTieneColumnaException si la tabla no tiene la columna que queremos
	 * @throws TiposNoCoincidenException si los tipos no coinciden
	 */
	public boolean setEstado(Estado estado) {
		try {
			DBManager.updateField(Tabla.OFERTA, Columna.OFERTA_ESTADO, this.id, estado.toString()); /** Cambiamos el estado en la base de datos*/
			this.estado = estado;
			return true;
		}catch(SQLException e) {
			e.printStackTrace();
	 ...
	 * @param opinion opinion que queremos añadir
	 */
	...
	public String toString() {
		return "[id = " + this.id + ", Estado:" + this.estado +", Fecha de inicio: " + fechaInicio + ", Precio: " + precio + ", fianza: " + fianza + ", descripcion: " + descripcion+
				", inmueble: " + inmueble + "]";
	}
	 ...
	 * @param demandante id del nuevo demandante
	 * @return true si se cambia correctamente, si no false
	 * @throws OfertaNoModificableException si la oferta no es editable
	 * @throws SQLException Si falla el SQL 
	 * @throws TablaNoTieneColumnaException si la tabla no tiene la columna que queremos
	 * @throws TiposNoCoincidenException si los tipos de datos no coinciden
	 */
	public boolean setDemandante(Integer demandante)throws OfertaNoModificableException, SQLException {
		if(this.estado == Estado.ACEPTADA) { /**Comprobamos que la oferta este en aceptada*/
			try {
				DBManager.updateField(Tabla.OFERTA, Columna.OFERTA_DEMANDANTE, this.id, demandante); /** Actualizamos el demandante en la tabla de datos*/
	...
	public static Oferta cargarOferta(int id) throws SQLException {
		...
		if(rs.next()) {
			String tipo = rs.getString(Columna.OFERTA_TIPO.toString()); /** Conseguimos el tipo para cargarla dependiendo del tipo*/
	 ...
	 * @param date nueva fecha final de la oferta
	 * @return true si se cambia correctamente, si no false
	 * @throws OfertaNoModificableException si la oferta no es modificable
	 */
	public boolean setFechaFin(LocalDate date) throws OfertaNoModificableException {return false;}
	 ...
	 */
	public boolean setNumMeses(int numMeses)throws OfertaNoModificableException{return false;}
	
	/**
	 * Devuelve el numero de meses
	 * @return Numero de meses de esta oferta
	 */
	public Integer getNumMeses() {return null;}
	
	/**
	 * Devuelve la fecha final de la oferta
	 * @return FechaFin fecha final de la oferta
	 */
	public LocalDate getFechaFin() {return null;}
